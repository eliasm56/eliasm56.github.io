---
title: Research
page-layout: full
---

# Arctic Circumpolar Permafrost Degradation Impacts

The estimated cost of building damage caused by projected permafrost degradation has been largely underestimated.
Why? The answer is simple:\
\
Our understanding of climate risk is only as good as our understanding of our exposure to hazards. 
The better we can account for what is at risk, the better we can measure risk in a changing world. Previous studies
relied on geospatial building data that (1) only measured the two-dimensional area of buildings and (2) did not account
for building occupancy. However, the cost of
replacing a totaled (i.e., damaged beyond repair) building depends on the total floor space, not just the footprint
area, as well as the purpose for which a building is designed.\
\
After detecting buildings from satellite imagery, classifying their occupancy, and estimating the total floor space
of residential buildings, projected building damage based on a geotechnical model amounts to **76 B USD under SSP2‐4.5
and 261 B USD under SSP5‐8.5**.\
\
See the methods used to map the building stock below.

## Residential Building Damages

The total residential floor area across the the Arctic circumpolar permafrost region amounts to an estimated
189 million sq. m, which is **75% greater** than the total footprint area of residential buildings (108 milion sq. m).
By mid-century, permafrost degradation could cause up to $261 billion in damage to the residential building stock.

![](assets/images/BC_overlay_LAEA_Residential_Building_Volume.png){width=100%}

![](assets\images\2panel_dam_cost_res_usd2024_p50_continuous.png){width=100%}

---

## Non-Residential Building Damages

Estimated non-residential building volume used for infrastructure damage assessment.

![](assets/images/BC_overlay_LAEA_NonResidential_Building_Volume.png){width=100%}

![](assets\images\2panel_dam_cost_nonres_usd2024_p50_continuous.png){width=100%}

---

# Mapping the Arctic Circumpolar Building Stock

## Building detection
![](assets\images\detection_workflow.PNG){width=100%}

## Building occupancy classification
![](assets\images\classification_workflow.PNG){width=100%}

## Residential building volume estimation
![](assets\images\volume_workflow.PNG){width=100%}